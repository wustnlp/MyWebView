...
/**
 * java与js之间可以进行互相通信，
 * 主动发起通信时，传输的数据我们称作request(请求数据)对应{@link Request},
 * 当把处理结果进行返回时的数据我们称作response(响应数据) * 对应{@link Response},
 * 因此该类的主要作用就是用来构建request或者response数据的。
 * 每次只能构建request或者response其中一种数据，
